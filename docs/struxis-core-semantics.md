...
`CBar` 由 `SBar` 按“包含关系合并”规则生成，目标是消除包含噪声，形成可用于分型识别的结构序列。CBar组成的k线序列相邻bar之间不会有包含的关系。
...
- 连续的顶分型不能相连，连续的底分型不能相连。
- 在说顶底时，上下文的含义是：顶指顶分型的最高k线的最高点，底指底分型的最低k线的最低点。

重叠/不重叠判定口径（关键约束）：
- 分型区间按“分型三根整体包络范围”计算，而非仅中间一根 `CBar`；
...
- `Extending`：当前方向正在延展；
- `Tentative`：已出现反向候选，等待后继 swing 证明其有效；
...
- `Confirmed`：实线；
- `Tentative`：虚线；
- `Extending`：虚线，但颜色要与Tentative有所区分。

---

## 5. Trend（趋势结构，同一周期中最大的几何结构）

`Trend` 以 `Swing` 为输入，将相邻 `Swing` 连接形成更高一级结构：

- 以上涨 `Swing`开始的Trend，也以上涨Swing结束，决不会以下降Swing结束，串联形成一个 `Trend`；
- 按方向分为 **上升趋势** 与 **下降趋势**。
