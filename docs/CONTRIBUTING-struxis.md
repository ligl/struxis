...
- [ ] 本次规则是否优先使用结构关系、状态机、顺序约束表达（而非阈值调参）
- [ ] 若引入阈值/参数：是否在 PR 描述中说明“为什么无法用结构语义表达”
- [ ] 若引入阈值/参数：是否提供回放或回归证据，证明该参数不是拍脑袋调参
- [ ] 是否说明参数失效边界（在哪些行情形态下可能不可靠）
- [ ] 是否评估“减参/去参”替代方案，并给出结论

默认要求：少用或不用阈值/参数；阈值仅作为有证据的例外机制，不能作为常规修复手段。
